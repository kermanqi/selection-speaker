...
YouTube CC、图片和其他不可选文字可以使用屏幕取词：

1. 在“偏好设置”中录制“屏幕取词（OCR）”快捷键，建议避开 Shottr 的截图快捷键。
2. 按下快捷键后拖拽框住字幕或单词；按 Esc 可取消。
3. App 会在本机识别中英文文字，并直接进入翻译流程。含中文的内容译成英文；纯英文内容译成中文。

首次使用时，macOS 会要求允许“屏幕录制”。截图只保留在内存中用于文字识别，不会保存到磁盘或剪贴板。屏幕取词不改变菜单栏当前的翻译方向，也不修改现有提示词。
